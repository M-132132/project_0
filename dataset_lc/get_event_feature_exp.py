...
def time_series_process(input_series, option):
    ...
    output_matrix = np.asarray(output_matrix, np.float32)
...
class FeatureConstruction(object):
    def __init__(self, hdd_file_number):
        ...
        self.tracks_meta_csv = read_tracks_static(f"{DATASET_ROOT}/data/{self.hdd_file_number}_tracksMeta.csv")
        ...
        for frame_num in range(1, len(self.tracks_csv[i][FRAME])):
            if self.tracks_csv[i][LANE_ID][frame_num] != self.tracks_csv[i][LANE_ID][frame_num-1]:
                ...
                while starting_change > last_boundary:
                    if LaneInfo._detect_lane_change(laneinfo.lanes_info[original_lane],
                                                    self.tracks_csv[i][Y][starting_change],
                                                    laneinfo.lane_width,
                                                    self.tracks_meta_csv[i][HEIGHT]):
                        break
                    starting_change -= 1
    ...
    def _get_features(self, lane_info_object, vehicle_id, frame_num, original_lane, option):
        ...
        if self.feature_option == "CNN_FC" and option == "ego_dop":
    ...
    def construct(self):
        ...
        if len(self.lane_keeping_ids) >= self.lane_change_count * LANE_CHANGE_KEEP_RATIO:
            ...
            self.lane_keeping_ids = self.lane_keeping_ids
    ...
    def construct_lanechange_features(self):
        ...
        for i in self.lane_changing_ids:
            ...
            for pair in changing_tuple_list:
                ...
                # 分别为换道开始的 frm, 换道结束的 frm, 方向 1 为从左往右开
                ...
                if not fail:
                    ...
                    cur_change_event = {"ego_dop": time_series_process(cur_change_ego_dop, "ego_dop"),
                                        ...
                                        "ego_vector": np.array(cur_change_ego_vector, dtype=np.float32),
                                        "other_info": [i, self.tracks_csv[i][FRAME][pair[0]], self.tracks_csv[i][FRAME][pair[1]]]}
        ...
        for i in self.lane_keeping_ids:
            ...
            try:
                ...
                for frame_num in range(1, FRAME_TAKEN+FRAME_CONSIST+1):
                    feature_ego_dop = self._get_features(laneinfo, i, frame_num, original_lane, "ego_dop")
                    feature_sur_dop = self._get_features(laneinfo, i, frame_num, original_lane, "sur_dop")
                    if frame_num <= FRAME_TAKEN:
                        cur_keep_ego_dop.append(feature_ego_dop)
                        cur_keep_sur_dop.append(feature_sur_dop)

                cur_change_ego_vector = self._get_features(laneinfo, i, FRAME_TAKEN, original_lane, "ego_vector")
            except Exception as error:
                # print(error)
...
def cnn_fc_main():
    if not os.path.exists("data_dop"):
        os.makedirs("data_dop")

    data_idx = 4
    number = "{0:0=2d}".format(data_idx)
    feature_construction = FeatureConstruction(number)
    feature_construction.construct()
    result, change_num, keep_num = \
        feature_construction.result, feature_construction.lane_change_count, feature_construction.lane_keep_count
    
    for subset_name, subset in zip(["data{}_exp".format(data_idx)], [result]):
        filename = f"data_dop/result_{subset_name}_{FEATURE_CHOICE}.pickle"
